...
import { TemplatePreview, parseMapBubbles } from '@/components/template-preview';
...
export function PinBoard({ pins = [], own, pending, onPlace, disabled = false, moodPoints, template = 'mood', layout = '' }: {
  pins?: BoardPin[]; own?: Point | null; pending?: Point | null;
  onPlace?: (point: Point) => void; disabled?: boolean; moodPoints?: MoodPoint[]; template?: QuestionTemplate; layout?: string;
}) {
  ...
  return <div className={`pin-canvas ${onPlace ? 'is-interactive' : 'is-display'}`}>
    <div className="board-art" aria-hidden="true">
      {template === 'mood' && <>{landmarks.map((position, index) => <div className="board-landmark" key={index} style={{ left: `${position.x}%`, top: `${position.y}%` }}>{moodPoints?<><span className="mood-emoji">{points[index].emoji}</span><span>{points[index].label}</span></>:<><ReactionFace choice={index} /><span>{choices[index].label}</span></>}</div>)}<span className="board-center-mark" /></>}
      {template !== 'mood' && <TemplatePreview template={template} bubbles={parseMapBubbles(layout)} />}
    </div>
