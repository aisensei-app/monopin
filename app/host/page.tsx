...
import { ArrowUpRight, RotateCcw, Users, Maximize, Minimize, Pencil, Copy, Check, Eye, EyeOff, List, Home, SlidersHorizontal, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function HostPage() {
  ...
  return (
    <main className="host-shell">
      <header className="host-header"><Wordmark href={homeUrl} /><span className="header-divider" /><span className="eyebrow">{data?.title || 'みんなのピンボード'}</span><div className="header-actions"><span className="anonymous-label">匿名で回答</span><span className={`connection ${error ? 'offline' : ''}`}><span />{error ? '接続を確認中' : data ? data.open ? '回答受付中' : '受付終了' : '接続中'}</span><button className="icon-button" onClick={toggleFullscreen} aria-label={fullscreen ? '全画面表示を終了' : '全画面で表示'}>{fullscreen ? <Minimize size={20} /> : <Maximize size={20} />}</button><AccountMenu /></div></header>
