...
export default function HostPage() {
  ...
  return (
    <main className="host-shell">
      ...
      {toolsOpen && <div className="host-tools-popover"><a href={roomUrl('editor',room)} className={data?.open ? 'is-blocked' : ''} aria-disabled={!!data?.open} onClick={(e) => { if (data?.open) e.preventDefault(); }}><List size={17}/>質問一覧</a><a href={homeUrl} className={data?.open ? 'is-blocked' : ''} aria-disabled={!!data?.open} onClick={(e) => { if (data?.open) e.preventDefault(); }}><Home size={17}/>部屋一覧・トップ</a><button onClick={toggleFullscreen}>{fullscreen?<Minimize size={17}/>:<Maximize size={17}/>}全画面表示</button>{data?.open && <div className="nav-blocked-note">受付中は画面を移動できません</div>}</div>}
