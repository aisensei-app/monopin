...
export default function HostPage() {
  ...
  return (
    <main className="host-shell">
      <header className="host-header"><Wordmark href={homeUrl} /><span className="header-divider" /><span className="eyebrow">{data?.title || 'みんなのピンボード'}</span><div className="header-actions"><span className="anonymous-label">匿名で回答</span><span className={`connection ${error ? 'offline' : ''}`}><span />{error ? '接続を確認中' : data ? data.ended ? '終了済み' : data.open ? '回答受付中' : '受付終了' : '接続中'}</span><AccountMenu /></div></header>
