    ...
    <main className="host-shell">
      ...
      <div className="host-layout">
        <section className="results-panel" aria-labelledby="host-question">
          <div className="host-meta-row"><span className="anonymous-label">匿名で回答</span><div className="total"><Users size={21} /><strong>{total}</strong><span>人が回答</span></div></div>
      ...
      <div className="host-dock" aria-label="主催者の操作"><button className="host-dock-main" disabled={!data || busy || mutating} onClick={()=>update('open')}>{data?.open ? '■ 受付を終了' : '▶ 回答を開始'}</button><div className="host-dock-icons"><button className="host-dock-icon" onClick={() => { setDraft(data?.question || QUESTION); setTitleDraft(data?.title || ''); setTitleMessage(''); setEditing(true); }} disabled={!data} aria-label="部屋と質問を編集" title="部屋と質問を編集"><Pencil size={19}/></button><button className="host-dock-icon" onClick={()=>update('visibility')} disabled={!data || busy} aria-label={data?.showAnswers === false ? '回答を表示する' : '回答を非表示にする'} title={data?.showAnswers === false ? '回答を表示する' : '回答を非表示にする'}>{data?.showAnswers === false ? <Eye size={20}/> : <EyeOff size={20}/>}</button><button className="host-dock-icon" onClick={() => setConfirm(true)} disabled={busy || !data || !total} aria-label="回答をリセット" title="回答をリセット"><RotateCcw size={19}/></button><div className="host-dock-icon-wrap"><button className="host-dock-icon" onClick={() => setToolsOpen(!toolsOpen)} aria-label="移動と設定" title="移動と設定"><SlidersHorizontal size={20}/></button>{toolsOpen && <div className="host-tools-popover is-anchored"><a href={roomUrl('editor',room)} className={data?.open ? 'is-blocked' : ''} aria-disabled={!!data?.open} onClick={(e) => { if (data?.open) e.preventDefault(); }}><List size={17}/>質問一覧</a><a href={homeUrl} className={data?.open ? 'is-blocked' : ''} aria-disabled={!!data?.open} onClick={(e) => { if (data?.open) e.preventDefault(); }}><Home size={17}/>部屋一覧・トップ</a><button onClick={toggleFullscreen}>{fullscreen?<Minimize size={17}/>:<Maximize size={17}/>}全画面表示</button>{data?.open && <div className="nav-blocked-note">受付中は画面を移動できません</div>}</div>}</div></div></div>
